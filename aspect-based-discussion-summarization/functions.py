...
def compute_sentiment_score(g):
    ...
    for n in g.nodes():
        ...
        tg_g.node[n]['sentiment_score'] = \
            float((g.node[n]['sentiment']['pos_count']-g.node[n]['sentiment']['neg_count'])/(
                    g.node[n]['sentiment']['pos_count']
                    +g.node[n]['sentiment']['neg_count']
                    + g.node[n]['sentiment']['neu_count']))
    return tg_g
...
#                       {}]
def read_comment_file(data_file, read_as_threads=False, load_2k=False):
